...
(function(addon) {
    ...
    if (typeof define == 'function' && define.amd) {
        define('uidata-slideset', ['uidata'], function(){
    ...
    UI.component('slideset', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('[data-ud-slideset]', context).each(function(){

                    var ele = UI.$(this);

                    if(!ele.data('slideset')) {
                        UI.slideset(ele, UI.Utils.options(ele.attr('data-ud-slideset')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.list      = this.element.find('.ud-slideset');
            this.nav       = this.element.find('.ud-slideset-nav');
            ...
            $this.list.addClass('ud-grid-width-1-'+$this.options.default);
            ...
            ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                ...
                $this.list.addClass('ud-grid-width-'+bp+'-1-'+$this.options[bp]);
            });

            this.on('click.ud.slideset', '[data-ud-slideset-item]', function(e) {
                ...
                var set = UI.$(this).attr('data-ud-slideset-item');
            ...
            this.controls.on('click.ud.slideset', '[data-ud-filter]', function(e) {

                var ele = UI.$(this);

                if (ele.parent().hasClass('ud-slideset')) {
                    return;
                }

                e.preventDefault();

                if ($this.animating || $this.currentFilter == ele.attr('data-ud-filter')) {
                    return;
                }

                $this.updateFilter(ele.attr('data-ud-filter'));
            ...
            UI.domObserve(this.list, function(e) {
                if ($this.list.children(':visible:not(.ud-active)').length) {
        ...
        update: function(animate, force) {
            ...
            if (this.nav.length && this.nav.empty()) {
                ...
                for (i=0;i<this.sets.length;i++) {
                    this.nav.append('<li data-ud-slideset-item="'+i+'"><a></a></li>');
                }

                this.nav[this.nav.children().length==1 ? 'addClass':'removeClass']('ud-invisible');
            }
        ...
        updateFilter: function(currentfilter) {
            ...
            this.controls.find('[data-ud-filter]').each(function(){

                filter = UI.$(this);

                if (!filter.parent().hasClass('ud-slideset')) {

                    if (filter.attr('data-ud-filter') == $this.currentFilter) {
                        filter.addClass('ud-active');
                    } else {
                        filter.removeClass('ud-active');
                    }
                }
            });
        },
        ...
        getVisibleOnCurrenBreakpoint: function() {
            ...
            var breakpoint  = null,
                ...
                ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                    ...
                    tmp.attr('class', 'ud-grid-width-'+bp+'-1-2').width();
        ...
        getItems: function() {
            ...
            if (this.currentFilter) {
                ...
                this.children.each(function(index){
                    ...
                    var ele = UI.$(this), f = ele.attr('data-ud-filter'), infilter = filter.length ? false : true;
        ...
        show: function(setIndex, noanimate, dir) {
            ...
            if (this.nav.length) {
                this.nav.children().removeClass('ud-active').eq(setIndex).addClass('ud-active');
            }
            ...
            animation.apply($this, [current, next, dir]).then(function(){
                ...
                $this.children.hide().removeClass('ud-active');
                next.addClass('ud-active').css({'display': '', 'opacity':''});
                ...
                $this.trigger('show.ud.slideset', [next]);
            });
    ...
    function coreAnimation(cls, current, next, dir) {
        ...
        release = function() {
            ...
            if (current && current.length) {
                current.hide().removeClass(clsOut+' ud-animation-reverse').css({'opacity':'', 'animation-delay': '', 'animation':''});
            }
            ...
            for (i=0;i<current.length;i++) {
                ...
                (function (index, ele){
                    ...
                    setTimeout(function(){
                        ...
                        }).addClass(clsOut+' ud-animation-reverse');
