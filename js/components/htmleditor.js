...
(function(addon) {
    ...
    if (typeof define == 'function' && define.amd) {
        define('uidata-htmleditor', ['uidata'], function(){
    ...
    UI.component('htmleditor', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('textarea[data-ud-htmleditor]', context).each(function() {

                    var editor = UI.$(this);

                    if (!editor.data('htmleditor')) {
                        UI.htmleditor(editor, UI.Utils.options(editor.attr('data-ud-htmleditor')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.content    = this.htmleditor.find('.ud-htmleditor-content');
            this.toolbar    = this.htmleditor.find('.ud-htmleditor-toolbar');
            this.preview    = this.htmleditor.find('.ud-htmleditor-preview').children().eq(0);
            this.code       = this.htmleditor.find('.ud-htmleditor-code');
            ...
            if (this.options.iframe) {
                ...
                this.iframe = UI.$('<iframe class="ud-htmleditor-iframe" frameborder="0" scrolling="auto" height="100" width="100%"></iframe>');
            ...
            this.htmleditor.on('click', '.ud-htmleditor-button-code, .ud-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.htmleditor.attr('data-mode') == 'tab') {

                    $this.htmleditor.find('.ud-htmleditor-button-code, .ud-htmleditor-button-preview').removeClass('ud-active').filter(this).addClass('ud-active');

                    $this.activetab = UI.$(this).hasClass('ud-htmleditor-button-code') ? 'code' : 'preview';
            ...
            this.on('init.ud.component', function() {
                $this.debouncedRedraw();
            });

            this.element.attr('data-ud-check-display', 1).on('display.ud.check', function(e) {
        ...
        _buildtoolbar: function() {
            ...
            this.options.toolbar.forEach(function(button) {
                ...
                bar.push('<li><a data-htmleditor-button="'+button+'" title="'+title+'" data-ud-tooltip>'+$this.buttons[button].label+'</a></li>');
            });
        ...
        fit: function() {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.ud-htmleditor-button-code, .ud-htmleditor-button-preview').removeClass('ud-active')
                    .filter(this.activetab == 'code' ? '.ud-htmleditor-button-code' : '.ud-htmleditor-button-preview')
                    .addClass('ud-active');
            }
    ...
    UI.components.htmleditor.template = [
        '<div class="ud-htmleditor ud-clearfix" data-mode="split">',
            '<div class="ud-htmleditor-navbar">',
                '<ul class="ud-htmleditor-navbar-nav ud-htmleditor-toolbar"></ul>',
                '<div class="ud-htmleditor-navbar-flip">',
                    '<ul class="ud-htmleditor-navbar-nav">',
                        '<li class="ud-htmleditor-button-code"><a>{:lblCodeview}</a></li>',
                        '<li class="ud-htmleditor-button-preview"><a>{:lblPreview}</a></li>',
                        '<li><a data-htmleditor-button="fullscreen"><i class="ud-icon-expand"></i></a></li>',
                    '</ul>',
                '</div>',
            '</div>',
            '<div class="ud-htmleditor-content">',
                '<div class="ud-htmleditor-code"></div>',
                '<div class="ud-htmleditor-preview"><div></div></div>',
            '</div>',
        '</div>'
    ].join('');
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.addButtons({
                ...
                fullscreen: {
                    title  : 'Fullscreen',
                    label  : '<i class="ud-icon-expand"></i>'
                },
                bold : {
                    title  : 'Bold',
                    label  : '<i class="ud-icon-bold"></i>'
                },
                italic : {
                    title  : 'Italic',
                    label  : '<i class="ud-icon-italic"></i>'
                },
                strike : {
                    title  : 'Strikethrough',
                    label  : '<i class="ud-icon-strikethrough"></i>'
                },
                blockquote : {
                    title  : 'Blockquote',
                    label  : '<i class="ud-icon-quote-right"></i>'
                },
                link : {
                    title  : 'Link',
                    label  : '<i class="ud-icon-link"></i>'
                },
                image : {
                    title  : 'Image',
                    label  : '<i class="ud-icon-picture-o"></i>'
                },
                listUl : {
                    title  : 'Unordered List',
                    label  : '<i class="ud-icon-list-ul"></i>'
                },
                listOl : {
                    title  : 'Ordered List',
                    label  : '<i class="ud-icon-list-ol"></i>'
                }
            ...
            editor.htmleditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                ...
                editor.htmleditor.toggleClass('ud-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.htmleditor.hasClass('ud-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            UI.$.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.htmleditor.find('.ud-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.htmleditor.find('.ud-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }
